function calc_switch_pdf(outputFilename, inputFilename, species, parameter, fp1, fp3, i, j, k, l, m)

if nargin < 9 || nargin > 11
    error('Usage: calc_switch_pdf(outputDirname, inputFilename, species, parameter, fp1, fp3, i, j, k, [l, m])');
end
...
pdata=cast(permute(hdf5read(inputFilename,sprintf('/Simulations/%07d/ParameterValues/%07d',1,parameter)),[2,1]),'double');
...
for x=[100:1:size(data,1)-1]
    if data(x,4) < fp1 && data(x+1,4) >= fp1
        sfpCrossings(end+1) = 0;
        sfpCrossingIndex(end+1) = x+1;
    elseif data(x,4) >= fp1 && data(x+1,4) < fp1
        sfpCrossings(end+1) = 0;
        sfpCrossingIndex(end+1) = x;
    elseif data(x,4) <= fp3 && data(x+1,4) > fp3
