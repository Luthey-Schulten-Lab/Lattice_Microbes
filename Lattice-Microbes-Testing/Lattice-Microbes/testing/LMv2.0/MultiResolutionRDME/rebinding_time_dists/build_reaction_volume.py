...
if len(sys.argv) != 3:
    quit("Usage: output_filename invivo")

outputFilename=sys.argv[1]
invivo=bool(sys.argv[2]=="1" or sys.argv[2]=="True")

xlen=448e-9
ylen=448e-9
zlen=448e-9
inVivoVolumeFraction=0.20
builder=LatticeBuilder(xlen,ylen,zlen,6.25e-9,1,0)
externalType=0
externalRegion=Cuboid(point(0.0,0.0,0.0), point(xlen,ylen,zlen), externalType)
builder.addRegion(externalRegion)
internalType=1
internalRegion=Capsule(point(xlen/2.0,ylen/2.0,zlen/2.0), point(xlen/2.0,ylen/2.0,zlen/2.0), 200e-9, internalType)
builder.addRegion(internalRegion)
obstacleType=2
...
lattice.addParticle(112,112,112,1+0)

# Add the repressor
for i in range(0,7):
    lattice.addParticle(112,112,112,1+1)
